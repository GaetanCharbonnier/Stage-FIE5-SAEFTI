...
int switchState  = 0; // Variable pour lecture de l'etat du switch (ON-OFF)
...
void setup() {
  ...
  //Initialisation du switch pin comme un input
...
//----------------------------------------------------------------------
 Serial.print(F("Initialisation de la SD card..."));
 if (!SD.begin(SDchipSelect)) {
  Serial.println();
  Serial.println(F("Echec de l'initialisation du lecteur SD card Vérifiez:"));
  Serial.println(F("* La bonne insertion de la carte"));
  Serial.println(F("* Le bon cablâge du module SD"));
  Serial.println(F("* que la vrariable du Pin 'cs' est correct "));
  Serial.println(F("Appuyé sur le bouton reset de l'aduino, pour redémarer le programme "));
  while (true);
  }
...
//-------------------------------------------------
// Lecture de l'etat de la valeur du switch (ON-OFF)
//-------------------------------------------------
...
//-------------------------------------------------
  ...
  //--------------------------------------------------------
  // Mise en formme de ces données (chiffre après la virgule)
  //--------------------------------------------------------
  String HCHOArrondie = String(HCHOppm,1); // Affichage d'une seule décimale
  //HCHOArrondie.replace(".",","); // Remplacement du séparateur
